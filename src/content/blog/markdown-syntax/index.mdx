...
### Public Image when base path is defined

The [method described above](#public-image) doesn't work if the website is hosted with a base path, e.g.: `https://rootdomain.com/subdirectory` (`subdirectory` is the `base` value set in `astro.config.mjs`).

To handle this case, you need to use MDX format and then `MdxPublicImage` component:
