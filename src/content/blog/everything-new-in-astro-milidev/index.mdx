...
Astro Milidev supports the base path. If you want to host your site in a "subdirectory", you can set the `base` property in the `astro.config.mjs` file.

This is useful when you want to host your site on e.g., GitHub Pages.
...
Navigate to the [Syntax guide](../content-and-syntax-guide) post to read how to handle [images](../content-and-syntax-guide#public-images) and [links](../syntax-guide#relative-links).

## Pagination

All collection supports pagination by default. On the collection page, you can navigate between pages using the pagination component. 
...
Each post can have its own OpenGraph image defined by `ogImage` property in the post metadata section. If no image is provided, the global OpenGraph image is used.

## Last update date

For updated posts, you can provide the last update date. It's displayed next to the publish date on the post page and in the buttons.
If `lastUpdateDate` property is defined, it takes precedence over `date` in sorting.

## Graphical logo

Astro Milidev allows to set up a graphical logo. The max dimenstions are 240px width and 50px height. 
...
Astro Milidev has the "dev" in the name and that's not only because it's tailored for the software engineers' needs.

I'm a backend engineer and refactor a lot of code that I encountered in the Astro Micro theme.

Sometimes it's just a small change, sometimes it's a complete rewrite. The changes contain but are not limited to:
